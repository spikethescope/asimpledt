...
label_encoders = {}
for column in data.columns[:-1]:  # Exclude the target column which is the last one
...
independentvars = data.drop(columns=['PlayTennis'])
target = data['PlayTennis']

# Splitting the dataset into training and testing data
X_train, X_test, y_train, y_test = train_test_split( independentvars, target , test_size=0.2, random_state=0)
...
plt.figure(figsize=(12, 8))
from sklearn.tree import plot_tree
plot_tree(clf, feature_names= X_train.columns.tolist(), class_names=['No', 'Yes'], filled=True, rounded=True)
plt.show()
